...
def clean_data(df_reviews):
    # Handle NaN values
    df_reviews['Description'] = df_reviews['Description'].fillna('')  # <--- Added this line to replace NaN with empty string
...
def train_data(df_reviews):
    ...
    df_reviews[['Sentiment', 'Confidence']] = df_reviews['Description'].apply(lambda x: analyze_sentiment_st(x) if isinstance(x, str) and x else ('Neutral', 0)).apply(pd.Series)
